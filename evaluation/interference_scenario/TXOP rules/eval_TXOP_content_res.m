...
%% EVAL
filen = 'BSS2_TXOPcontentRestriction_results.mat';
load(filen)
...
y2axis = results.latency.retry.TXOPcontentRestriction_data./1e-6;
...
hist_h = histogram(results.latency.retry.data(min_lim:max_lim)./1e-6 - results.latency.retry.TXOPcontentRestriction_data(min_lim:max_lim)./1e-6 ,hist_retry_cor_lat_x,'Normalization','cdf');
...
xlabel('latency [탎]')
ylabel('CDF')
legend('Congestion','Aggregation','Transmit','Ack','Retry','Retry w/ TXOP content restriction','Location','SouthEast')
...
xaxis = results.latency.xaxis;
yaxis = results.latency.retry.data./1e-6 - results.latency.retry.TXOPcontentRestriction_data./1e-6;
...
xlabel('Sequence number')
ylabel('Latency [탎]')
legend('Congestion','Aggregation','Transmit','Ack','Retry','Retry w/ content restriction','Location','NorthWest')
...
lat_noAck_wShort = results.latency.congestion.data./1e-6 + results.latency.aggregation.data./1e-6 + results.latency.retry.data./1e-6 - results.latency.retry.TXOPcontentRestriction_data./1e-6;
lat_ack_wShort = results.latency.congestion.data./1e-6 + results.latency.aggregation.data./1e-6 + results.latency.retry.data./1e-6 + results.latency.ack.data./1e-6 - results.latency.retry.TXOPcontentRestriction_data./1e-6;
...
legend('w/o Ack','w/ Ack','w/o Ack w/ content restricion','w/ Ack w/ content restriction')
...
ylabel('CDF')
lh = legend('w/o Ack','w/ Ack','w/o Ack w/ TXOP content restriction','w/ Ack w/ TXOP content restriction');
...
legend('avg','min','max','avg w/ content restriction','min w/ content restriction','max w/ content restriction')
%%
...
xlabel('Percentile');
lh = legend('w/o Ack w/o content restriction', 'w/o Ack w/ content restriction', 'w/ Ack w/o content restriction', 'w/ Ack w/ content restriction');
...
set(gca,'XTick',1:length(percentile));
set(gca,'XTickLabel',percentile*100);
ylabel('Gain of TXOP content restriction in end-to-end latency [ms]')
xlabel('Percentile');
grid on
...
set(gca,'XTick',1:length(percentile));
set(gca,'XTickLabel',percentile*100);
ylabel('Gain of TXOP content restriction in end-to-end latency [%]')
xlabel('Percentile');
grid on
lh = legend('w/o Ack', 'w/ Ack');
set(lh,'Location','NorthWest')
