...
load('ap2.mat');
dest_mac = '00:00:00_00:00:02 (00:00:00:00:00:02) (RA)'; %'00:00:00_00:00:01 (00:00:00:00:00:01) (RA)'
dest_ip = '192.168.1.2'; %'192.168.1.1';
...
% fix A-MPDU
offset = 0;
for c = 1:length(data_BSS1)
    tmp = cell2mat(data_BSS1(c,8));
    if isempty(tmp)
        offset = offset + 1;
        data_BSS1{c,8} = data_BSS1{c-1,8} + 1;
    else
        data_BSS1{c,8} = tmp + offset;
    end
end
    ...
    if ~isempty(OBSS_ampdu_no)
        ...
        if half_ind~=0
            ...
            for d = 1:length(sn_failedAMPDU)
                ...
                if ~isempty(shortend_dur)
                    
                    if ~isempty(ind)
                        ind = ind(end);
                        cur_fn = IBSS_data{ind,1};
                        lat_advantage = sum(shortend_dur(fn_firstMPDUinOBSS_AMPDU<cur_fn));
                        retry_lat_BSS1_correction = [retry_lat_BSS1_correction; IBSS_data{ind,1} lat_advantage];
                    end
                end
            end
            ...
            fn_add = cell2mat(data_BSS1(cell2mat(data_BSS1(:,1))>cur_fn & cell2mat(data_BSS1(:,8))==AMPDU_ref_no, 1));
            
            retry_lat_BSS1_correction = [retry_lat_BSS1_correction; fn_add lat_advantage.*ones(length(fn_add),1)];
        end
    end
...
close(h);
save ap2_TXOPshortening.mat retry_lat_BSS1_correction
