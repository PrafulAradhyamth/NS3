...
filen = 'BSS2_results.mat'; % BSS2_results.mat
load('BSS2_raw.mat'); % BSS2_raw.mat
load('BSS2_TXOPcontentrestriction.mat')
...
for c = 1:length(seq_number_unwrap)
    if retry(c)==1
        ...
        fn = frame_number(c);
        cand_corr_terms = retry_lat_BSS2_correction(retry_lat_BSS2_correction(:,1)==fn,2);
...
results.latency.retry.TXOPcontentRestriction_data = retry_lat_reduction;

save([filen(1:end-4) '_TXOPcontentRestriction.mat'], 'results');
%%
figure(1)
yyaxis left
